...
import {
  ...
  Map, BarChart3, Settings, PanelLeftClose, PanelLeftOpen,
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 h-16 border-b flex-shrink-0" style={{ borderColor: "var(--border)" }}>
        ...
        {!collapsed && (
          <div>
            <p className="text-sm font-semibold leading-none" style={{ color: "var(--foreground)" }}>Reservas Pro</p>
            <p className="text-xs mt-0.5" style={{ color: "var(--foreground-muted)" }}>Gestão de Mesas</p>
          </div>
        )}
      </div>
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href}
              ...
              className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all group", active ? "font-medium" : "hover:opacity-80")}
              style={{
